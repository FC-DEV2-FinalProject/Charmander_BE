...
public class AuthService {
    ...
    public LoginResponse login(OAuthUserInfo userInfo) {
        var principalId = MemberPrincipal.createScopedPrincipalId(userInfo.provider(), userInfo.id());
        var member = memberRepository.findByPrincipal_IdAndPrincipal_Type(principalId, MemberPrincipalType.OAUTH);
        if (member == null) {
            throw new CoreApiException(CoreApiExceptionCode.AUTH_INVALID_CREDENTIAL);
        }
        var accessToken = jwtService.createAccessToken(member);
